...
module.exports = (input, options) => {
  ...
  parsed = _.map(parsed, p => {
    ...
    // Only keep recent two years
    ['finances', 'employees'].forEach(s => {
      _.each(p[s], (v, year) => {
        if (!~[y, y - 1].indexOf(parseInt(year, 10))) {
          delete p[s][year];
        }
      });
    });
  ...
  // console.error('coid,name,founded,incorporated,link');
  // _.each(parsed, p => {
  //   console.error(
  //     `${p.coid},"${p.name || ''}",${p.founded || ''},${p.inc ||
  //       ''},"http://newstools.star.local/data-ui/admin/business_companies/company/${
  //       p.coid
  //     }"`
  //   );
  // });
